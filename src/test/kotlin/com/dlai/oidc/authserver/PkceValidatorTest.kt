...
class PkceValidatorTest {
    ...
    fun `valid verifier against its own S256 challenge succeeds`() {
        ...
        assertTrue(PkceValidator.verify(verifier, challenge, "S256"))
    }

    @Test
    fun `mismatched verifier fails`() {
        val challenge = challengeFor("the-real-verifier")
        assertFalse(PkceValidator.verify("a-different-verifier", challenge, "S256"))
    }
    ...
    fun `plain method is rejected even with a matching value`() {
        ...
        assertFalse(PkceValidator.verify(value, value, "plain"))
    }

    @Test
    fun `unknown method is rejected`() {
        val challenge = challengeFor("some-verifier")
        assertFalse(PkceValidator.verify("some-verifier", challenge, "S512"))
    }

    @Test
    fun `empty verifier fails`() {
        val challenge = challengeFor("some-verifier")
        assertFalse(PkceValidator.verify("", challenge, "S256"))
    }
    ...
    fun `verifier less than minimum length fails`() {
        ...
        val challenge = challengeFor("some-verifier")
        assertFalse(PkceValidator.verify(verifier, challenge, "S256"))
    }
    ...
    fun `verifier greater than maximum length fails`() {
        val challenge = challengeFor("some-verifier")
        assertFalse(PkceValidator.verify("a".repeat(129), challenge, "S256"))
    }
